...
static func regeneration() -> EffectDefinition:
	...
	effect.duration = 8.0; effect.tick_interval = 1.0; effect.heal_per_tick = 5.0
	effect.stack_policy = EffectDefinition.StackPolicy.REFRESH
...
static func haste() -> EffectDefinition:
	...
	effect.stat_modifiers = [
		StatModifierDefinition.new(C_AttackSpeed, R_ModifiesStat.Operation.INCREASED, 0.25),
		StatModifierDefinition.new(C_CastSpeed, R_ModifiesStat.Operation.INCREASED, 0.25),
		StatModifierDefinition.new(C_CooldownRecovery, R_ModifiesStat.Operation.INCREASED, 0.25),
	]
...
static func slow() -> EffectDefinition:
	...
	effect.stat_modifiers = [StatModifierDefinition.new(C_MoveSpeed, R_ModifiesStat.Operation.MORE, 0.60)]
