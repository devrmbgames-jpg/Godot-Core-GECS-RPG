...
static func resolve(actor: Entity, ability: Entity, target: Entity, target_position: Vector3, command_buffer: CommandBuffer) -> void:
	...
	var raw_damage: float = _calculate_raw_damage(actor, definition)
...
static func _resolve_melee(actor: Entity, ability: Entity, target: Entity, definition: AbilityDefinition, raw_damage: float) -> void:
	...
	var direction: Vector3 = _resolve_direction(actor, target, Vector3.ZERO)
	...
	DamageService.request(victim, DamageRequest.new(
		...
		definition.damage_type, definition.status_buildup_scale, elemental_context,
	))
	ElementalService.request_statuses(
		victim, definition.elemental_statuses, actor, ability, hit.position, direction, elemental_context,
	)
